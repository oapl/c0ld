-- Recover exact final global ranks when a player exists in the compact member
-- archive but was not copied into the per-player global archive. The finalized
-- candidate distribution remains authoritative; the archived member points are
-- only used to place that player within the same ordering (points desc, ID asc).
...
as $$
  with raw_ranked as materialized (
  ...
  ), missing_compact as materialized (
    select
      final.*,
      coalesce(final.global_points, final.clan_points) as ranking_points,
      coalesce(
        nullif(final.source_run_key, ''),
        nullif(manifest.final_global_run_key, '')
      ) as ranking_run_key
    from public.c0ld_battle_player_finals final
    left join public.c0ld_battle_final_runs manifest
      on lower(manifest.battle_key) = lower(final.battle_key)
    where final.user_id = p_user_id
      and final.global_rank is null
      and coalesce(final.global_points, final.clan_points) is not null
  ), reconstructed_archived as (
    select
      missing.ranking_run_key as run_key,
      missing.final_snapshot_at as fetched_at,
      missing.battle_display_name as event_name,
      missing.battle_key,
      missing.battle_display_name,
      missing.clan_name,
      distribution.global_rank,
      missing.ranking_points::bigint as global_points,
      distribution.total_global_players,
      missing.member_rank as clan_rank,
      missing.clan_points,
      true as found,
      lower(missing.battle_key) as battle_identity,
      1 as source_order
    from missing_compact missing
    join lateral (
      select
        (
          1 + count(*) filter (
            where candidate.points > missing.ranking_points
               or (
                 candidate.points = missing.ranking_points
                 and candidate.user_id < missing.user_id
               )
          )
        )::integer as global_rank,
        count(*)::integer as total_global_players
      from public.c0ld_global_rank_candidates candidate
      where candidate.run_key = missing.ranking_run_key
      having count(*) > 0
    ...
    union all
    select * from reconstructed_archived
    union all
...
comment on function public.get_c0ld_retained_global_battle_history(text, bigint) is
  'Returns raw final ranks first and reconstructs missing compact ranks from the preserved final candidate distribution.';
