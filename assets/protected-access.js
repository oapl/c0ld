(function () {
  ...
  async function requireAccess(config) {
    ...
    if (callback.denied) {
      setText(statusId, "Your Discord account does not have the required role for this page.");
      return { allowed: false, reason: "denied" };
    }

    const token = getToken();
    if (!token) {
      setText(statusId, "Sign in with Discord to verify your role before this page loads.");
