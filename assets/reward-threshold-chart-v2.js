(() => {
        ...
        <div class="reward-threshold-controls">
          ...
          <div class="reward-threshold-metrics">
            <select id="reward-threshold-window" aria-label="Growth interval window">
              ${Object.entries(WINDOWS).map(([key, option]) => `<option value="${key}"${key === selectedWindow ? " selected" : ""}>${option.label}</option>`).join("")}
            </select>
  ...
  async function loadData(force = false) {
    ...
    try {
      ...
      const historyUrl = `${HISTORY_URL}?battle=${encodeURIComponent(battle)}&hours=${windowConfig.hours}&rank_min=${range.min}&rank_max=${range.max}&bucket_minutes=${windowConfig.bucketMinutes}&include_baseline=1&limit=50000`;
  ...
  function rowsForWindow(rows, clanRow, windowConfig, endMs) {
    ...
    if (finite(clanRow.points) !== null && clanRow.fetched_at) {
      output.push({ fetched_at: clanRow.fetched_at, rank: clanRow.rank, clan_name: clanRow.clan_name, points: clanRow.points });
    }

    return dedupePullRows(output);
  }

  function buildDataPullPoints(rows, windowConfig, endMs, clanRow) {
    ...
    for (let i = 1; i < cleanRows.length; i += 1) {
      ...
      const averagedGain = rawGain / Math.max(1, gapMs / bucketMs);

      points.push({
        pullIndex: points.length,
        rawT: new Date(curr.ms).toISOString(),
        previousT: new Date(prev.ms).toISOString(),
        pointsGained: averagedGain,
        rawPointsGained: rawGain,
  ...
  function buildSeries() {
    ...
    return selectedRows().map((row, index) => {
      const points = buildDataPullPoints(historyForClan(row.clan_name), windowConfig, endMs, row);
      return {
  ...
  function draw() {
    ...
    const x = point => maxPulls <= 1 ? padLeft : padLeft + (point.pullIndex / (maxPulls - 1)) * width;
    ...
    ctx.fillStyle = "#8b949e";
    ctx.fillText("Data pulls", padLeft, rect.height - 12);
    const lastLabel = `${maxPulls} pulls`;
    ...
    if (legend) {
      ...
      legend.innerHTML = legendItems.map(series => `<span class="reward-threshold-legend-item" style="color:${series.color}">#${series.rank} ${escapeHtml(series.clan_name)}</span>`).join("") + `<span>${maxPulls} data-pull intervals · gains averaged to ${windowConfig.bucketMinutes}m</span>`;
    }

    canvas._rewardThresholdChart = { seriesList, x, y, padTop, padBottom, rect, windowConfig, maxPulls };
  ...
  function bindTooltip(canvas, tooltip) {
    ...
    function move(ev) {
      ...
      const samePullRows = fresh.seriesList
        .map(series => ({ series, point: series.points.find(point => point.pullIndex === nearestPoint.pullIndex) }))
        .filter(item => item.point)
        .sort((a, b) => a.series.rank - b.series.rank);

      tooltip.innerHTML = `<strong>${escapeHtml(fmtDateTime(nearestPoint.rawT))}</strong>${samePullRows.map(({ series, point }) => `<div class="tooltip-row" style="color:${series.color}">${escapeHtml(series.clan_name)} Total Points: ${fmtShort(point.totalPoints)} / Points Gained: ${fmtShort(point.pointsGained)} avg (${Math.round(point.gapMinutes)}m gap)</div>`).join("")}`;
