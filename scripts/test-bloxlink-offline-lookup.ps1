...
Write-Host "Checking the exact Bloxlink mapping used for offline mentions. This does not post an alert or request BIG Games data..."
$result = Invoke-RestMethod `
  -Method Post `
  -Uri ("{0}/api/offline/bloxlink/lookup" -f $WorkerUrl.TrimEnd('/')) `
...
if (-not $result.ok) {
  Write-Host "`nNo usable Bloxlink mapping was returned. Inspect bloxlink.http_status and bloxlink.error above." -ForegroundColor Yellow
}
