...
const OUTPUT_FILE = path.join(DATA_DIR, "c0ld-history.json");
    ...
    try {
      const json = await requestJsonWithRetry(
        url,
        {
          headers: {
            Accept: "application/json",
            "User-Agent": "c0ld-Leaderboard-Avatar-Enrichment"
          }
        },
...
async function downloadAvatarToCache(cacheKey, imageUrl) {
  ...
  try {
    const res = await fetch(imageUrl, {
      headers: {
        "User-Agent": "c0ld-Leaderboard-History-Avatar-Cache"
      }
    });
...
async function main() {
  ...
  const declared = manualBattles
    .filter(battle => battle.battle && battle.display_name && battle.c0ld_results_table)
    .map(battle => ({
      battle: String(battle.battle),
      display_name: String(battle.display_name),
      source_table: String(battle.c0ld_results_table),
      ...
      c0ld_results_table: String(battle.c0ld_results_table)
    }));
  ...
  for (const item of deduped.values()) {
    console.log(`Reading historical c0ld table: ${item.source_table} -> ${item.battle}`);
    ...
    output.push({
      ...
      c0ld_results_table: item.c0ld_results_table,
  ...
  console.log(`Historical c0ld battles written: ${output.length}`);
