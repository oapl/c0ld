...
; Tap position used by the Lightning counters and shown in the Fireworks clip.
CLICK_X := 851
CLICK_Y := 536
...
; Fireworks begins with a white/cyan flash over this otherwise solid-blue floor
; patch. The supplied recording has no pre-proc white pixels in this region,
; while the flash covers all 20x20 pixels before the rainbow particles spread.
DETECT_LEFT := 858
DETECT_TOP := 491
DETECT_RIGHT := 877
DETECT_BOTTOM := 510
FIREWORK_FLASH_COLOR := 0xE8FFFF
FIREWORK_FLASH_COLOR_VARIATION := 50
DETECTION_SCAN_INTERVAL_MS := 15
DETECTION_CONFIRM_SCANS := 2
CLEAR_SCANS_REQUIRED := 12
...
if A_Args.Length > 0 && A_Args[1] = "--capture-test" {
    ...
    TestResult := FireworksVisible()
    TestElapsed := A_TickCount - TestStarted
    FileAppend "visible=" TestResult " elapsed_ms=" TestElapsed "`n", "*"
    ExitApp
}

OutputFile := A_Desktop "\fireworks-procs-" FormatTime(A_Now, "yyyyMMdd-HHmmss") ".txt"
FileAppend "Fireworks:`n", OutputFile, "UTF-8"

CounterGui := Gui("+AlwaysOnTop +ToolWindow", "Fireworks Counter")
CounterGui.BackColor := "211018"
CounterGui.MarginX := 16
CounterGui.MarginY := 14
CounterGui.SetFont "s10 cFFF1F6", "Segoe UI"

CounterGui.AddText "xm ym w125", "Status"
StatusValue := CounterGui.AddText("x+8 yp w145", "Stopped")
StatusValue.SetFont "s10 bold cFF5A8A", "Segoe UI"
...
CounterGui.AddText "xm y+10 w125", "Fireworks signal"
SignalValue := CounterGui.AddText("x+8 yp w145", "-")
SignalValue.SetFont "s10 bold cFF9DBA", "Segoe UI"

CounterGui.AddText "xm y+12 w278 h32 cC9A4B0", "Output: " RegExReplace(OutputFile, "^.*\\", "")
...
StartCounter() {
    ...
    ClearScanCount := 0
    ButtonPrimed := false
    ProcLatched := CanReadRobloxScreen() ? FireworksVisible() : false
    StatusValue.Text := REQUIRE_ROBLOX_ACTIVE && !CanReadRobloxScreen()
        ? "Waiting for Roblox"
        : "Running"

    SetTimer CheckForFireworks, DETECTION_SCAN_INTERVAL_MS
...
FireworksVisible() {
    ...
    global FIREWORK_FLASH_COLOR, FIREWORK_FLASH_COLOR_VARIATION
    ...
    return PixelSearch(
        ...
        DETECT_RIGHT,
        DETECT_BOTTOM,
        FIREWORK_FLASH_COLOR,
        FIREWORK_FLASH_COLOR_VARIATION
    )
}
