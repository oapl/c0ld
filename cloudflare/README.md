...
| `HTG_SCHEDULE_ALIGNMENT_MINUTES` | Optional. Defaults to `15`. HTG uses a stable, per-account minute inside every 15-minute window, rather than sending every member at the same wall-clock second. This keeps a 150-member community under BIG Games' per-key minute limit while preserving predictable cadence. A confirmed VIP account uses one assigned minute in every window only when its quota reserve is `0`. |
| `HTG_DISABLE_SCHEDULE_STAGGER` | Diagnostic-only opt-out. Defaults to `false`. Do not set it in production: setting it to `true` makes every HTG scan fall at the same quarter-hour minute and can rate-limit a community-sized tracker. |
...
| `HTG_REQUIRE_SOURCE_FILTER` | Optional. Defaults to `true`. Known transfers are always suppressed. A gain confirmed by BIG Games' first-owner record posts immediately; only ownership-unknown gains remain pending for trade/booth/mail verification or a second fresh inventory revision. |
...
| `HTG_DISABLE_FORCED_REFRESH` | Diagnostic-only opt-out. Do not set this in production. HTG requests a fresh BIG Games inventory revision at its quota-safe scheduled cadence; comparing cached revisions cannot discover a newly acquired HTG. The former `HATCH_FORCE_REFRESH_ON_SCHEDULE` setting is ignored for HTG compatibility and may be removed. |
...
The Worker gates clan and league delivery to UTC minute `:00`, with `:01` and `:02` (and the hourly retry window up through `:03`) reserved as retries; user boards may retry through `:05` while still rendering only the exact `:00` and prior-hour `:00` samples. Hourly boards become due at `HOURLY_CLAN_POST_MINUTE` and then drain through a fair queue on later minute ticks: different Discord channels can post in parallel while multiple boards sharing one channel are serialized to avoid per-channel rate limits. The default queue attempts up to two distinct channels per minute; set `HOURLY_SCHEDULED_BATCH_SIZE` to change that cap. The Worker suppresses clan logs and persistent clan trackers during UTC `:00`–`:03`; clan logs default to every 15 minutes from UTC `:05` and persistent clan trackers default to every 20 minutes from UTC `:11`.
