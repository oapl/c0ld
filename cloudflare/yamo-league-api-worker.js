...
export default {
  ...
  async scheduled(event, env, ctx) {
    ctx.waitUntil((async () => {
      ...
      if (String(env.INGEST_TOP_LEAGUES || "true").toLowerCase() !== "false") await handleTopLeaguesIngest(env, "schedule");
    })());
  }
};
...
async function handleTopLeaguesIngest(env, source) {
  ...
  const dbRows = top.rows.map(row => ({ snapshot_id: snapshotId, fetched_at: fetchedAt, source: `${source}:top-leagues`, league_name: TOP_LEAGUES_NAME, league_id: row.league_id || null, league_level: row.league_level ?? null, league_points: row.points || 0, league_icon: row.league_icon || null, member_capacity: row.member_capacity ?? null, rank: row.rank, user_id: row.user_id, display_name: row.league_name, points: row.points, last_contribution_at: null, permission_level: null, role: "Top League", join_time: null, raw_member: { league_name: row.league_name, league_id: row.league_id, league_rank: row.rank }, raw_contribution: {}, raw_league: row.raw_league }));
  if (dbRows.length) { await supabaseInsert(env, SNAPSHOT_TABLE, dbRows); await replaceCurrentRows(env, CURRENT_TABLE, { league_name: `eq.${TOP_LEAGUES_NAME}` }, dbRows.map(row => ({ ...row, updated_at: fetchedAt }))); }
...
async function handleTopLeagues(request, env) {
  ...
  let rows = await supabaseSelect(env, CURRENT_TABLE, { select: "snapshot_id,fetched_at,source,league_name,league_id,league_level,league_points,league_icon,member_capacity,rank,user_id,display_name,points,last_contribution_at,permission_level,role,join_time,raw_league", league_name: `eq.${TOP_LEAGUES_NAME}`, order: "rank.asc", limit: String(limit) });
  if (!rows.length && String(env.TOP_LEAGUES_LIVE_FALLBACK || "true").toLowerCase() !== "false") {
    const live = await fetchTopLeagues(limit);
    const now = new Date().toISOString();
    rows = live.rows.map(row => ({ snapshot_id: "live", fetched_at: now, source: "live", league_name: TOP_LEAGUES_NAME, league_id: row.league_id, league_level: row.league_level, league_points: row.points, league_icon: row.league_icon, member_capacity: row.member_capacity, rank: row.rank, user_id: row.user_id, display_name: row.league_name, points: row.points, role: "Top League", raw_league: row.raw_league }));
  }
  const latest = latestMeta(rows);
  ...
  const publicRows = rowsWithGains.map(row => { const out = publicLeagueRow(row); out.projected_gain_1h = projectGain1h(out); out.projected_points_1h = out.total_points + out.projected_gain_1h; return out; });
  publicRows.slice().sort((a, b) => b.projected_points_1h - a.projected_points_1h || a.rank - b.rank).forEach((row, index) => { row.projected_rank_1h = index + 1; });
  return cacheJson({ ok: true, generated_at: new Date().toISOString(), snapshot_at: latest.fetched_at, league_name: TOP_LEAGUES_NAME, source: "ps99-league-api-worker", projection: "Projected rank uses current league points plus the best available 1-hour-equivalent gain.", rows: publicRows }, env);
}
...
async function fetchTopLeagues(limit) {
  const api = await fetchLeagueListApi();
  const leagues = extractLeagueObjects(api.data || api).map((item, index) => { const summary = summarizeLeague(item, lname(item) || `League ${index + 1}`); const explicitRank = toNumber(firstDefined(item.Rank, item.rank, item.Place, item.place, item.Position, item.position)); return { item, index, summary, explicitRank }; }).filter(x => x.summary.league_name);
  leagues.sort((a, b) => (b.summary.league_points - a.summary.league_points) || (a.explicitRank || 999999) - (b.explicitRank || 999999) || a.summary.league_name.localeCompare(b.summary.league_name));
  const rows = leagues.slice(0, limit).map((x, index) => { const rank = x.explicitRank && x.explicitRank > 0 ? x.explicitRank : index + 1; const stable = x.summary.league_id || x.summary.league_name; const synthetic = stableLeagueUserId(stable); return { rank, user_id: synthetic, league_name: x.summary.league_name, league_id: x.summary.league_id, league_level: x.summary.league_level, league_icon: x.summary.league_icon, member_capacity: x.summary.member_capacity, points: x.summary.league_points, raw_league: { ...x.summary.raw_league, league_rank: rank, synthetic_user_id: synthetic } }; });
  return { source: "league-list", rows };
}

function extractLeagueObjects(value, out = []) { if (!value || typeof value !== "object") return out; if (Array.isArray(value)) { value.forEach(x => extractLeagueObjects(x, out)); return out; } const hasName = value.Name !== undefined || value.name !== undefined || value.LeagueName !== undefined || value.leagueName !== undefined; const hasPoints = value.Points !== undefined || value.points !== undefined || value.TotalPoints !== undefined || value.totalPoints !== undefined || value.Score !== undefined || value.score !== undefined; if (hasName && hasPoints) out.push(value); Object.values(value).forEach(x => extractLeagueObjects(x, out)); return out; }
function summarizeLeague(league, fallbackName) { const rawLeague = { Name: firstDefined(league.Name, league.name, league.LeagueName, league.leagueName, fallbackName), ID: firstDefined(league.ID, league.Id, league.id), Level: firstDefined(league.Level, league.level), Points: firstDefined(league.Points, league.points, league.TotalPoints, league.totalPoints, league.Score, league.score), Icon: firstDefined(league.Icon, league.icon), MemberCapacity: firstDefined(league.MemberCapacity, league.memberCapacity), Created: firstDefined(league.Created, league.created), Owner: firstDefined(league.Owner, league.owner) }; return { league_name: String(rawLeague.Name || fallbackName).trim() || fallbackName, league_id: stringOrNull(rawLeague.ID), league_level: toNumber(rawLeague.Level), league_points: toNumber(rawLeague.Points) || 0, league_icon: stringOrNull(rawLeague.Icon), member_capacity: toNumber(rawLeague.MemberCapacity), raw_league: rawLeague }; }
...
async function resolveRobloxUsernames(userIds, env) { const shouldLookup = String(env.ROBLOX_USERNAME_LOOKUPS || "true").toLowerCase() !== "false"; const result = new Map(); const ids = [...new Set(userIds.map(Number).filter(Boolean))]; for (const id of ids) result.set(id, `user_${id}`); if (!shouldLookup || !ids.length) return result; const lookupBatch = async batch => { try { const res = await fetch("https://users.roblox.com/v1/users", { method: "POST", headers: { Accept: "application/json", "Content-Type": "application/json", "User-Agent": "c0ld-League-API-Worker" }, body: JSON.stringify({ userIds: batch, excludeBannedUsers: false }) }); if (!res.ok) return; const data = await res.json(); for (const user of data.data || []) { const id = toNumber(user.id); if (id && user.name) result.set(id, String(user.name)); } } catch {} }; await Promise.all(chunk(ids, ROBLOX_BATCH_SIZE).map(lookupBatch)); return result; }
async function resolveRobloxAvatarHeadshots(userIds, env) { const shouldLookup = String(env.ROBLOX_AVATAR_LOOKUPS || "true").toLowerCase() !== "false"; const result = new Map(); const ids = [...new Set(userIds.map(Number).filter(Boolean))]; if (!shouldLookup || !ids.length) return result; for (const batch of chunk(ids, ROBLOX_BATCH_SIZE)) { const url = new URL("https://thumbnails.roblox.com/v1/users/avatar-headshot"); url.searchParams.set("userIds", batch.join(",")); url.searchParams.set("size", "150x150"); url.searchParams.set("format", "Png"); url.searchParams.set("isCircular", "false"); try { const res = await fetch(url.toString(), { headers: { Accept: "application/json", "User-Agent": "c0ld-League-API-Worker" } }); if (!res.ok) continue; const data = await res.json(); for (const item of data.data || []) { const id = toNumber(item?.targetId); const imageUrl = String(item?.imageUrl || "").trim(); const state = String(item?.state || "").trim(); if (id && imageUrl && state === "Completed") result.set(id, imageUrl); } } catch {} } return result; }

function publicMemberRow(row, usernameMap, avatarMap) { const id = toNumber(row.user_id); const name = displayUsername(row, usernameMap); return { fetched_at: row.fetched_at, rank: toNumber(row.rank), user_id: id, username: name, display_name: name, avatar_url: avatarMap.get(String(id)) || null, total_points: toNumber(row.points) || 0, points: toNumber(row.points) || 0, last_contribution_at: row.last_contribution_at || null, permission_level: row.permission_level ?? null, role: row.role || "Member", join_time: row.join_time || null, gain_5m: row.gain_5m, gain_1h: row.gain_1h, gain_6h: row.gain_6h, gain_12h: row.gain_12h, gain_24h: row.gain_24h }; }
function publicLeagueRow(row) { const raw = row.raw_league || {}; const name = String(firstDefined(raw.Name, raw.name, raw.LeagueName, raw.leagueName, row.display_name) || "").trim(); return { fetched_at: row.fetched_at, rank: toNumber(row.rank), synthetic_id: toNumber(row.user_id), league_name: name, display_name: name, league_id: stringOrNull(firstDefined(raw.ID, raw.Id, raw.id, row.league_id)), league_level: toNumber(firstDefined(raw.Level, raw.level, row.league_level)), league_icon: stringOrNull(firstDefined(raw.Icon, raw.icon, row.league_icon)), member_capacity: toNumber(firstDefined(raw.MemberCapacity, raw.memberCapacity, row.member_capacity)), total_points: toNumber(row.points) || 0, points: toNumber(row.points) || 0, gain_5m: row.gain_5m, gain_1h: row.gain_1h, gain_6h: row.gain_6h, gain_12h: row.gain_12h, gain_24h: row.gain_24h }; }
...
async function addGainFields(env, rows, latest) { if (!rows.length) return []; const latestMs = new Date(latest.fetched_at).getTime(); if (!Number.isFinite(latestMs)) return rows.map(addNullGains); const windows = [{ key: "gain_5m", minutes: 5, tolerance: 4 }, { key: "gain_1h", minutes: 60, tolerance: 10 }, { key: "gain_6h", minutes: 360, tolerance: 20 }, { key: "gain_12h", minutes: 720, tolerance: 25 }, { key: "gain_24h", minutes: 1440, tolerance: 45 }]; const maps = {}; for (const win of windows) maps[win.key] = await fetchClosestPointMap(env, latest.league_name, latestMs, win.minutes, win.tolerance); return rows.map(row => { const out = { ...row }; const currentPoints = toNumber(row.points) || 0; for (const win of windows) { const previous = maps[win.key].get(String(row.user_id)); out[win.key] = previous === undefined ? null : currentPoints - previous; } return out; }); }
