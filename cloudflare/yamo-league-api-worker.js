  ...
  async fetch(request, env) {
    try {
      ...
      if (request.method === "GET" && url.pathname === "/api/health") {
        response = json({ ok: true, service: "ps99-league-api", league_name: leagueName(env), league_names: leagueNames(env), snapshot_retention: "permanent" });
...
async function handleCurrent(request, env) {
  ...
  if (!latest) return cacheJson({ generated_at: new Date().toISOString(), snapshot_at: null, league_name: requested, rows: [] }, env);
  ...
  return cacheJson({
    ...
    rows: rowsWithGains.map(row => {
      ...
      return {
        ...
        username: name,
        display_name: name,
        avatar_url: avatarMap.get(String(id)) || null,
...
async function handleHistory(request, env) {
  ...
  return cacheJson({ generated_at: new Date().toISOString(), league_name: requested, hours: hoursParam || 24, rows }, env);
}
...
async function fetchLeagueApi(league) {
  ...
  for (const url of urls) {
    try {
      const res = await fetch(url, { headers: { Accept: "application/json", "User-Agent": "c0ld-League-API-Worker" }, cf: { cacheTtl: 0, cacheEverything: false } });
...
function summarizeLeague(league, fallbackName) {
  const rawLeague = { Name: firstDefined(league.Name, league.name, fallbackName), ID: firstDefined(league.ID, league.Id, league.id), Level: firstDefined(league.Level, league.level), Points: firstDefined(league.Points, league.points), Icon: firstDefined(league.Icon, league.icon), MemberCapacity: firstDefined(league.MemberCapacity, league.memberCapacity), Created: firstDefined(league.Created, league.created), Owner: firstDefined(league.Owner, league.owner) };
...
async function resolveRobloxUsernames(userIds, env) {
  ...
  const lookupBatch = async batch => {
    try {
      const res = await fetch("https://users.roblox.com/v1/users", { method: "POST", headers: { Accept: "application/json", "Content-Type": "application/json", "User-Agent": "c0ld-League-API-Worker" }, body: JSON.stringify({ userIds: batch, excludeBannedUsers: false }) });
      if (!res.ok) return;
      const data = await res.json();
      for (const user of data.data || []) {
        const id = toNumber(user.id);
        if (id && user.name) result.set(id, String(user.name));
      }
    } catch {}
  };
  const work = [];
  for (let i = 0; i < ids.length; i += ROBLOX_BATCH_SIZE) work.push(lookupBatch(ids.slice(i, i + ROBLOX_BATCH_SIZE)));
...
async function resolveRobloxAvatarHeadshots(userIds, env) {
  ...
  for (let i = 0; i < ids.length; i += ROBLOX_BATCH_SIZE) {
    ...
    try {
      const res = await fetch(url.toString(), { headers: { Accept: "application/json", "User-Agent": "c0ld-League-API-Worker" } });
      if (!res.ok) continue;
      const data = await res.json();
      for (const item of data.data || []) {
        ...
        if (id && imageUrl && state === "Completed") result.set(id, imageUrl);
      }
    } catch {}
  }
  return result;
