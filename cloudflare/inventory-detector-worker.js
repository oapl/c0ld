...
async function handleIngest(request, env, source) {
  ...
  const is_boundary = parseBool(url.searchParams.get("boundary")) ?? isMountainMidnight(new Date(), env);
  const result = await ingestInventory(env, user, source, is_boundary);
...
async function ingestInventory(env, user, source, isBoundary) {
  ...
  const itemRows = items.map(item => normalizeItemRow(item, snapshot.id, userId, fetchedAt, localDay));
  for (const chunk of chunks(itemRows, 500)) {
    if (chunk.length) await supabaseInsert(env, ITEM_TABLE, chunk, "minimal");
  }

  return { snapshot: lightSnapshot(snapshot), item_count: itemRows.length };
}
...
function pickComparisonSnapshots(descSnapshots, requestedDay) {
  ...
  if (requestedDay) {
    const dayStart = snaps.find(s => s.local_day === requestedDay && s.is_boundary) || snaps.find(s => s.local_day === requestedDay);
    const nextDay = nextLocalDate(requestedDay);
    const dayEnd = snaps.find(s => s.local_day === nextDay && s.is_boundary) || [...snaps].reverse().find(s => s.local_day === requestedDay || s.local_day === nextDay);
    if (dayStart && dayEnd && dayStart.id !== dayEnd.id) return { start: dayStart, end: dayEnd, mode: "requested_day" };
  }

  const boundary = snaps.filter(s => s.is_boundary);
  if (boundary.length >= 2) {
    return { start: boundary[boundary.length - 2], end: boundary[boundary.length - 1], mode: "midnight_to_midnight" };
  }
...
function buildDiff(startItems, endItems) {
  const startMap = new Map(startItems.map(i => [i.item_key, i]));
  const endMap = new Map(endItems.map(i => [i.item_key, i]));
  const added = [];
  const removed = [];
  const changed = [];
  ...
  for (const [key, end] of endMap.entries()) {
    ...
    const endCount = Number(end.count || 0);
    const startCount = start ? Number(start.count || 0) : 0;
    const delta = endCount - startCount;
    if (!start && endCount !== 0) added.push(diffRow(end, 0, endCount, delta));
    else if (delta !== 0) changed.push(diffRow(end, startCount, endCount, delta));
  }

  for (const [key, start] of startMap.entries()) {
    if (!endMap.has(key)) {
      const startCount = Number(start.count || 0);
      removed.push(diffRow(start, startCount, 0, -startCount));
    }
  }
...
async function fetchPublicInventory(usernameOrId) {
  ...
  let jsonData;
  try { jsonData = JSON.parse(text); } catch { throw httpError(502, `Big Games returned non-JSON: ${text.slice(0, 160)}`); }
  if (!res.ok || jsonData.status === "error") throw httpError(502, `Big Games inventory fetch failed: ${JSON.stringify(jsonData).slice(0, 260)}`);
  return jsonData;
}
...
function normalizeItemRow(item, snapshotId, userId, capturedAt, localDay) {
  ...
  const key = getItemKey(item, itemClass, itemId, variant);
  return {
...
function requireAdmin(request, env) {
  ...
  if (got !== `Bearer ${expected}`) throw httpError(401, "Unauthorized");
}
...
function localDateString(date, env) {
  const fmt = new Intl.DateTimeFormat("en-CA", { timeZone: timeZone(env), year: "numeric", month: "2-digit", day: "2-digit" });
